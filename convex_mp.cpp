...
void divide_and_conquer(deque<Point> &q, int leaves = 0 )
{
    ...
    if(leaves <= 1)
    {
        ...
        if(s2.size() > 1){
            divide_and_conquer(s2, leaves);  // work on the left queue
        }
    ...
    else
    {
        ...
        pipe(fd);
        int right = fork();
        if(right > 0) // parent process, handle left side
        {
            if (s1.size() > 1)
            {
                if(leaves % 2 == 1){
                    divide_and_conquer(s1, (leaves + 1) / 2);  // round up & work on the left queue
                }
                else
                {
        ...
        else  //child process, handle right side
        {
            if(s2.size() > 1)
            {
                if(leaves % 2 == 1)
                {
                    divide_and_conquer(s2, (leaves - 1) / 2);  // round down & work on the right queue
                }
                else // even leaves
                {
        ...
        s2.clear();    // make sure the right side is empty to receive from the child process
        wait(0);
        recv(fd[0], s2);
